...
Tovuti is an open source project, meaning everyone can use it and contribute to its development. When you join our community, you'll find a friendly group of enthusiasts at all experience levels who are willing to chat about anything and everything related to Tovuti.
...
The [issue tracker](https://github.com/tovutifunk/tovuti/issues) is for bug reports, feature requests, and pull requests.

- Please **do not** use the issue tracker for personal support requests. Use [the discussion forum](https://github.com/tovutifunk/tovuti/discussions/categories/help) instead.
...
If you would like your feature prioritized, please consider [sponsoring the project](https://github.com/sponsors/tovutifunk).
...
If you would like your bug prioritized, please consider [sponsoring the project](https://github.com/sponsors/tovutifunk).
...
`current` - This branch reflects the latest release and powers [prodkt.cloud](https://prodkt.cloud/).

`next` - This is the branch you should submit pull requests against. It reflects what's coming in the _next_ release, which can be previewed at [next.prodkt.cloud](https://next.prodkt.cloud/).

## Developing

To set up a local dev environment, [fork the repo](https://github.com/tovutifunk/tovuti/fork) on GitHub, clone it locally, and install its dependencies.

```bash
git clone https://github.com/YOUR_GITHUB_USERNAME/tovuti
cd tovuti
npm install
```

Once you've cloned the repo, run the following command to spin up the Tovuti dev server.
...
[![Open in Gitpod](../assets/images/open-in-gitpod.svg)](https://gitpod.io/#https://github.com/tovutifunk/tovuti)

### Creating New Components

To scaffold a new component, run the following command, replacing `moyo-tag-name` with the desired tag name.

```bash
npm run create moyo-tag-name
```
...
Tovuti uses [Web Test Runner](https://modern-web.dev/guides/test-runner/getting-started/) for testing. To launch the test runner during development, open a terminal and launch the dev server.
...
Tovuti is built with accessibility in mind. Creating generic components that are fully accessible to users with varying capabilities across a multitude of circumstances is a daunting challenge. Oftentimes, the solution to an a11y problem is not written in black and white and, therefore, we may not get it right the first time around. There are, however, guidelines we can follow in our effort to make Tovuti an accessible foundation from which applications and websites can be built.

We take this commitment seriously, so please ensure your contributions have this goal in mind. If you need help with anything a11y-related, please [reach out to the community](/resources/community) for assistance. If you discover an accessibility concern within the library, please file a bug on the [issue tracker](https://github.com/tovutifunk/tovuti/issues).
...
Maintaining good documentation can be a painstaking task, but poor documentation leads to frustration and makes the project less appealing to users. Fortunately, writing documentation for Tovuti is fast and easy!

Most of Tovuti's technical documentation is generated with JSDoc comments and TypeScript metadata from the source code. Every property, method, event, etc. is documented this way. In-code comments encourage contributors to keep the documentation up to date as changes occur so the docs are less likely to become stale. Refer to an existing component to see how JSDoc comments are used in Tovuti.
...
The `<moyo-select>` component, for example, makes use of the dropdown, input, menu, and menu item components. Because it's offloading most of its functionality and styles to lower-level components, the select component remains lightweight and its appearance is consistent with other form controls and menus.

### Component Structure

All components have a host element, which is a reference to the `<moyo-*>` element itself. Make sure to always set the host element's `display` property to the appropriate value depending on your needs, as the default is `inline` per the custom element spec.
...
Components must only emit custom events, and all custom events must start with `moyo-` as a namespace. For compatibility with frameworks that utilize DOM templates, custom events must have lowercase, kebab-style names. For example, use `moyo-change` instead of `slChange`.
...
Then use the following syntax for comments so they appear in the generated docs. Do not use the `--moyo-` prefix, as that is reserved for design tokens that live in the global scope.
 ...
 */
@customElement('moyo-example')
export default classMoyoExample {
  // ...
}
```
...
render() {
  return html`
    <div part="base">
      <moyo-icon part="icon" exportparts="base:icon__base" ...></moyo-icon>
    </div>
  `;
}
```
