...
Tovuti components can be customized at a high level through design tokens. This gives you control over theme colors and general styling. For more advanced customizations, you can make use of component parts and custom properties to target individual components.

## Design Tokens

Tovuti makes use of several design tokens to provide a consistent appearance across components. You can customize them and use them in your own application with pure CSS — no preprocessor required.
...
Design tokens are accessed through CSS custom properties that are defined in your theme. Because design tokens live at the page level, they're prefixed with `--moyo-` to avoid collisions with other libraries.
...
:root {
  ...
  --moyo-color-primary-50: var(--moyo-color-purple-50);
  --moyo-color-primary-100: var(--moyo-color-purple-100);
  --moyo-color-primary-200: var(--moyo-color-purple-200);
  --moyo-color-primary-300: var(--moyo-color-purple-300);
  --moyo-color-primary-400: var(--moyo-color-purple-400);
  --moyo-color-primary-500: var(--moyo-color-purple-500);
  --moyo-color-primary-600: var(--moyo-color-purple-600);
  --moyo-color-primary-700: var(--moyo-color-purple-700);
  --moyo-color-primary-800: var(--moyo-color-purple-800);
  --moyo-color-primary-900: var(--moyo-color-purple-900);
  --moyo-color-primary-950: var(--moyo-color-purple-950);
}
```

Many design tokens are described further along in this documentation. For a complete list, refer to `src/themes/light.styles.ts` in the project's [source code](https://github.com/tovutifunk/tovuti/blob/current/src/themes/light.styles.ts).
...
Tovuti components use a [shadow DOM](https://developer.mozilla.org/en-US/docs/Web/Web_Components/Using_shadow_DOM) to encapsulate their styles and behaviors. As a result, you can't simply target their internals with the usual CSS selectors. Instead, components expose "parts" that can be targeted with the [CSS part selector](https://developer.mozilla.org/en-US/docs/Web/CSS/::part), or `::part()`.

Here's an example that modifies buttons with the `tomato-button` class.

```html preview
<moyo-button class="tomato-button"> Tomato Button </moyo-button>

<style>
  .tomato-button::part(base) {
    background: var(--moyo-color-neutral-0);
...
For convenience, some components expose CSS custom properties you can override. These are not design tokens, nor do they have the same `--moyo-` prefix since they're scoped to a component.
...
```html
<moyo-avatar style="--size: 6rem;"></moyo-avatar>
```
...
```js
import { setDefaultAnimation } from '@tovutifunk/tovuti/dist/utilities/animation-registry.js';
...
```js
import { setAnimation } from '@tovutifunk/tovuti/dist/utilities/animation-registry.js';
