...
(() => {
  ...
  function convertModuleLinks(html) {
    const version = sessionStorage.getItem('moyo-version');

    html = html
      .replace(/@tovutifunk\/tovuti/g, `https://cdn.skypack.dev/@tovutifunk/tovuti@${version}`)
    ...
    if (button?.classList.contains('code-block__button--codepen')) {
      ...
      if (isReact) {
        ...
        jsTemplate =
          ...
          `import { setBasePath } from 'https://cdn.skypack.dev/@tovutifunk/tovuti@${version}/dist/utilities/base-path';\n` +
          `\n` +
          `// Set the base path for Tovuti assets\n` +
          `setBasePath('https://cdn.skypack.dev/@tovutifunk/tovuti@${version}/dist/')\n` +
      ...
      // CSS templates
      cssTemplate =
        `@import 'https://cdn.jsdelivr.net/npm/@tovutifunk/tovuti@${version}/dist/themes/${
          isDark ? 'dark' : 'light'
        }.css';\n` +
        '\n' +
        'body {\n' +
        ...
        '  color: var(--moyo-color-neutral-900);\n' +
        '  padding: 1rem;\n' +
        '}';
      ...
      const data = {
        title: '',
        description: '',
        tags: ['tovuti', 'web components'],
        editors,
        head: `<meta name="viewport" content="width=device-width">`,
        html_classes: `moyo-theme-${isDark ? 'dark' : 'light'}`,
        css_external: ``,
        js_external: ``,
        js_module: true,
