...
import { registerIconLibrary } from '../../../dist/tovuti.js';
import type MoyoIcon from './icon';
...
describe('<moyo-icon>', () => {
  ...
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<MoyoIcon>(html` <moyo-icon></moyo-icon> `);
    ...
    it('renders pre-loaded system icons and emits moyo-load event', async () => {
      const el = await fixture<MoyoIcon>(html` <moyo-icon library="system"></moyo-icon> `);
      const listener = oneEvent(el, 'moyo-load');
    ...
    it('the icon is accessible', async () => {
      const el = await fixture<MoyoIcon>(html` <moyo-icon library="system" name="check-lg"></moyo-icon> `);
  ...
  describe('new library', () => {
    it('renders icons from the new library and emits moyo-load event', async () => {
      const el = await fixture<MoyoIcon>(html` <moyo-icon library="test-library"></moyo-icon> `);
      const listener = oneEvent(el, 'moyo-load');
    ...
    it('runs mutator from new library', async () => {
      const el = await fixture<MoyoIcon>(html` <moyo-icon library="test-library" name="test-icon1"></moyo-icon> `);
      await elementUpdated(el);
  ...
  describe('negative cases', () => {
    ...
    it("svg not rendered with an icon that doesn't exist in the library", async () => {
      const el = await fixture<MoyoIcon>(html` <moyo-icon library="test-library" name="does-not-exist"></moyo-icon> `);

      expect(el.shadowRoot?.querySelector('svg')).to.be.null;
    });

    it('emits moyo-error when the file cant be retrieved', async () => {
      const el = await fixture<MoyoIcon>(html` <moyo-icon library="test-library"></moyo-icon> `);
      const listener = oneEvent(el, 'moyo-error');
    ...
    it("emits moyo-error when there isn't an svg element in the registered icon", async () => {
      const el = await fixture<MoyoIcon>(html` <moyo-icon library="test-library"></moyo-icon> `);
      const listener = oneEvent(el, 'moyo-error');
