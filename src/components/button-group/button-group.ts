...
const BUTTON_CHILDREN = ['moyo-button', 'moyo-radio-button'];

/**
 * @since Beta-46
 * @status stable
 *
 * @slot - One or more `<moyo-button>` elements to display in the button group.
 *
 * @csspart base - The component's internal wrapper.
 */
@customElement('moyo-button-group')
export default class MoyoButtonGroup extends LitElement {
  ...
  handleFocus(event: CustomEvent) {
    ...
    button?.classList.add('moyo-button-group__button--focus');
  }

  handleBlur(event: CustomEvent) {
    const button = findButton(event.target as HTMLElement);
    button?.classList.remove('moyo-button-group__button--focus');
  }

  handleMouseOver(event: CustomEvent) {
    const button = findButton(event.target as HTMLElement);
    button?.classList.add('moyo-button-group__button--hover');
  }

  handleMouseOut(event: CustomEvent) {
    const button = findButton(event.target as HTMLElement);
    button?.classList.remove('moyo-button-group__button--hover');
  }
...
declare global {
  interface HTMLElementTagNameMap {
    'moyo-button-group': MoyoButtonGroup;
  }
}
