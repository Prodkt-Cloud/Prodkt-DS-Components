...
import type MoyoAvatar from './avatar';

describe('<moyo-avatar>', () => {
  let el: MoyoAvatar;

  describe('when provided no parameters', () => {
    before(async () => {
      el = await fixture<MoyoAvatar>(html` <moyo-avatar label="Avatar"></moyo-avatar> `);
    });
  ...
  describe('when provided an image and label parameter', () => {
    ...
    before(async () => {
      el = await fixture<MoyoAvatar>(html`<moyo-avatar image="${image}" label="${label}"></moyo-avatar>`);
    });

    it('should pass accessibility tests', async () => {
      /**
       * The image element itself is ancillary, because it's parent container contains the
       * aria-label which dictates what "moyo-avatar" is. This also implies that label text will
  ...
  ['square', 'rounded', 'circle'].forEach(shape => {
    describe(`when passed a shape attribute ${shape}`, () => {
      before(async () => {
        el = await fixture<MoyoAvatar>(html`<moyo-avatar shape="${shape}" label="Shaped avatar"></moyo-avatar>`);
      });
  ...
  it('should not render the image when the image fails to load', async () => {
    ...
    el = await fixture<MoyoAvatar>(html`<moyo-avatar></moyo-avatar>`);
