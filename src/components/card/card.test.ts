...
import type MoyoCard from './card';
...
describe('<moyo-card>', () => {
  ...
  describe('when provided no parameters', () => {
    before(async () => {
      el = await fixture<MoyoCard>(
        html` <moyo-card>This is just a basic card. No image, no header, and no footer. Just your content.</moyo-card> `
      );
    });
