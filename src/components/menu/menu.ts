 ...
 *
 * @event {{ item: MoyoMenuItem }} moyo-select - Emitted when a menu item is selected.
 *
 * @csspart base - The component's internal wrapper.
 */
@customElement('moyo-menu')
export default class MoyoMenu extends LitElement {
  ...
  getAllItems(options: { includeDisabled: boolean } = { includeDisabled: true }) {
    return [...this.defaultSlot.assignedElements({ flatten: true })].filter((el: HTMLElement) => {
      ...
      if (!options.includeDisabled && (el as MoyoMenuItem).disabled) {
        return false;
      }
   ...
   */
  setCurrentItem(item: MoyoMenuItem) {
  ...
  typeToSelect(event: KeyboardEvent) {
    ...
    if (!hasFocusVisible) {
      items.forEach(item => item.classList.remove('moyo-focus-invisible'));
    }
  ...
  handleClick(event: MouseEvent) {
    ...
    const item = target.closest('moyo-menu-item');

    if (item?.disabled === false) {
      emit(this, 'moyo-select', { detail: { item } });
    }
  }
  ...
  handleKeyUp() {
    ...
    if (!hasFocusVisible) {
      ...
      items.forEach(item => {
        item.classList.remove('moyo-focus-invisible');
      });
    }
  }
  ...
  handleMouseDown(event: MouseEvent) {
    ...
    if (target.getAttribute('role') === 'menuitem') {
      this.setCurrentItem(target as MoyoMenuItem);

      // Hide focus in browsers that don't support :focus-visible when using the mouse
      if (!hasFocusVisible) {
        target.classList.add('moyo-focus-invisible');
      }
    }
  }
