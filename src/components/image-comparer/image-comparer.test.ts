...
import type MoyoImageComparer from './image-comparer';

describe('<moyo-image-comparer>', () => {
  it('should render a basic before/after', async () => {
    const el = await fixture<MoyoImageComparer>(html`
      <moyo-image-comparer>
        <div slot="before"></div>
        <div slot="after"></div>
      </moyo-image-comparer>
  ...
  it('should emit change event when position changed manually', async () => {
    ...
    el.addEventListener('moyo-change', handler, { once: true });
  ...
  it('should set position by attribute', async () => {
    const el = await fixture<MoyoImageComparer>(html`
      <moyo-image-comparer position="10">
        <div slot="before"></div>
        <div slot="after"></div>
      </moyo-image-comparer>
    `);
