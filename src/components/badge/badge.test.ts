...
import type MoyoBadge from './badge';
...
describe('<moyo-badge>', () => {
  ...
  describe('when provided no parameters', () => {
    before(async () => {
      el = await fixture<MoyoBadge>(html` <moyo-badge>Badge</moyo-badge> `);
    });
  ...
  describe('when provided a pill parameter', () => {
    before(async () => {
      el = await fixture<MoyoBadge>(html` <moyo-badge pill>Badge</moyo-badge> `);
    });
  ...
  describe('when provided a pulse parameter', () => {
    before(async () => {
      el = await fixture<MoyoBadge>(html` <moyo-badge pulse>Badge</moyo-badge> `);
    });
  ...
  ['primary', 'success', 'neutral', 'warning', 'danger'].forEach(variant => {
    describe(`when passed a variant attribute ${variant}`, () => {
      before(async () => {
        el = await fixture<MoyoBadge>(html`<moyo-badge variant="${variant}">Badge</moyo-badge>`);
      });
