...
import type MoyoFormatBytes from './format-bytes';

describe('<moyo-format-bytes>', () => {
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<MoyoFormatBytes>(html` <moyo-format-bytes></moyo-format-bytes> `);
  ...
  describe('bits', () => {
    ...
    results.forEach(expected => {
      it('bits : display formats', async () => {
        const el = await fixture<MoyoFormatBytes>(html` <moyo-format-bytes unit="bit"></moyo-format-bytes> `);
        // short
        el.value = expected.value;
        await elementUpdated(el);
        expect(el.shadowRoot?.textContent).to.equal(expected.short);
