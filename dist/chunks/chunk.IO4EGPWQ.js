import {
  form_control_styles_default
} from "./chunk.3BZPJXKY.js";
import {
  component_styles_default
} from "./chunk.ZOHC4QYC.js";
import {
  r
} from "./chunk.IXU25QGK.js";

// src/components/input/input.styles.ts
var input_styles_default = r`
  ${component_styles_default}
  ${form_control_styles_default}

  :host {
    display: block;
  }

  .input {
    flex: 1 1 auto;
    display: inline-flex;
    align-items: stretch;
    justify-content: start;
    position: relative;
    width: 100%;
    font-family: var(--moyo-input-font-family);
    font-weight: var(--moyo-input-font-weight);
    letter-spacing: var(--moyo-input-letter-spacing);
    vertical-align: middle;
    overflow: hidden;
    cursor: text;
    transition: var(--moyo-transition-fast) color, var(--moyo-transition-fast) border,
      var(--moyo-transition-fast) box-shadow, var(--moyo-transition-fast) background-color;
  }

  /* Standard inputs */
  .input--standard {
    background-color: var(--moyo-input-background-color);
    border: solid var(--moyo-input-border-width) var(--moyo-input-border-color);
  }

  .input--standard:hover:not(.input--disabled) {
    background-color: var(--moyo-input-background-color-hover);
    border-color: var(--moyo-input-border-color-hover);
  }

  .input--standard.input--focused:not(.input--disabled) {
    background-color: var(--moyo-input-background-color-focus);
    border-color: var(--moyo-input-border-color-focus);
    box-shadow: var(--moyo-focus-ring);
  }

  .input--standard.input--focused:not(.input--disabled) .input__control {
    color: var(--moyo-input-color-focus);
  }

  .input--standard.input--disabled {
    background-color: var(--moyo-input-background-color-disabled);
    border-color: var(--moyo-input-border-color-disabled);
    opacity: 0.5;
    cursor: not-allowed;
  }

  .input--standard.input--disabled .input__control {
    color: var(--moyo-input-color-disabled);
  }

  .input--standard.input--disabled .input__control::placeholder {
    color: var(--moyo-input-placeholder-color-disabled);
  }

  /* Filled inputs */
  .input--filled {
    border: none;
    background-color: var(--moyo-input-filled-background-color);
    color: var(--moyo-input-color);
  }

  .input--filled:hover:not(.input--disabled) {
    background-color: var(--moyo-input-filled-background-color-hover);
  }

  .input--filled.input--focused:not(.input--disabled) {
    background-color: var(--moyo-input-filled-background-color-focus);
    box-shadow: var(--moyo-focus-ring);
  }

  .input--filled.input--disabled {
    background-color: var(--moyo-input-filled-background-color-disabled);
    opacity: 0.5;
    cursor: not-allowed;
  }

  .input__control {
    flex: 1 1 auto;
    font-family: inherit;
    font-size: inherit;
    font-weight: inherit;
    min-width: 0;
    height: 100%;
    color: var(--moyo-input-color);
    border: none;
    background: none;
    box-shadow: none;
    padding: 0;
    margin: 0;
    cursor: inherit;
    -webkit-appearance: none;
  }

  .input__control::-webkit-search-decoration,
  .input__control::-webkit-search-cancel-button,
  .input__control::-webkit-search-results-button,
  .input__control::-webkit-search-results-decoration {
    -webkit-appearance: none;
  }

  .input__control:-webkit-autofill,
  .input__control:-webkit-autofill:hover,
  .input__control:-webkit-autofill:focus,
  .input__control:-webkit-autofill:active {
    box-shadow: 0 0 0 var(--moyo-input-height-large) var(--moyo-input-background-color-hover) inset !important;
    -webkit-text-fill-color: var(--moyo-color-primary-500);
    caret-color: var(--moyo-input-color);
  }

  .input--filled .input__control:-webkit-autofill,
  .input--filled .input__control:-webkit-autofill:hover,
  .input--filled .input__control:-webkit-autofill:focus,
  .input--filled .input__control:-webkit-autofill:active {
    box-shadow: 0 0 0 var(--moyo-input-height-large) var(--moyo-input-filled-background-color) inset !important;
  }

  .input__control::placeholder {
    color: var(--moyo-input-placeholder-color);
    user-select: none;
  }

  .input:hover:not(.input--disabled) .input__control {
    color: var(--moyo-input-color-hover);
  }

  .input__control:focus {
    outline: none;
  }

  .input__prefix,
  .input__suffix {
    display: inline-flex;
    flex: 0 0 auto;
    align-items: center;
    cursor: default;
  }

  .input__prefix ::slotted(moyo-icon),
  .input__suffix ::slotted(moyo-icon) {
    color: var(--moyo-input-icon-color);
  }

  /*
   * Size modifiers
   */

  .input--small {
    border-radius: var(--moyo-input-border-radius-small);
    font-size: var(--moyo-input-font-size-small);
    height: var(--moyo-input-height-small);
  }

  .input--small .input__control {
    height: calc(var(--moyo-input-height-small) - var(--moyo-input-border-width) * 2);
    padding: 0 var(--moyo-input-spacing-small);
  }

  .input--small .input__clear,
  .input--small .input__password-toggle {
    width: calc(1em + var(--moyo-input-spacing-small) * 2);
  }

  .input--small .input__prefix ::slotted(*) {
    padding-left: var(--moyo-input-spacing-small);
  }

  .input--small .input__suffix ::slotted(*) {
    padding-right: var(--moyo-input-spacing-small);
  }

  .input--medium {
    border-radius: var(--moyo-input-border-radius-medium);
    font-size: var(--moyo-input-font-size-medium);
    height: var(--moyo-input-height-medium);
  }

  .input--medium .input__control {
    height: calc(var(--moyo-input-height-medium) - var(--moyo-input-border-width) * 2);
    padding: 0 var(--moyo-input-spacing-medium);
  }

  .input--medium .input__clear,
  .input--medium .input__password-toggle {
    width: calc(1em + var(--moyo-input-spacing-medium) * 2);
  }

  .input--medium .input__prefix ::slotted(*) {
    padding-left: var(--moyo-input-spacing-medium);
  }

  .input--medium .input__suffix ::slotted(*) {
    padding-right: var(--moyo-input-spacing-medium);
  }

  .input--large {
    border-radius: var(--moyo-input-border-radius-large);
    font-size: var(--moyo-input-font-size-large);
    height: var(--moyo-input-height-large);
  }

  .input--large .input__control {
    height: calc(var(--moyo-input-height-large) - var(--moyo-input-border-width) * 2);
    padding: 0 var(--moyo-input-spacing-large);
  }

  .input--large .input__clear,
  .input--large .input__password-toggle {
    width: calc(1em + var(--moyo-input-spacing-large) * 2);
  }

  .input--large .input__prefix ::slotted(*) {
    padding-left: var(--moyo-input-spacing-large);
  }

  .input--large .input__suffix ::slotted(*) {
    padding-right: var(--moyo-input-spacing-large);
  }

  /*
   * Pill modifier
   */

  .input--pill.input--small {
    border-radius: var(--moyo-input-height-small);
  }

  .input--pill.input--medium {
    border-radius: var(--moyo-input-height-medium);
  }

  .input--pill.input--large {
    border-radius: var(--moyo-input-height-large);
  }

  /*
   * Clearable + Password Toggle
   */

  .input__clear,
  .input__password-toggle {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    font-size: inherit;
    color: var(--moyo-input-icon-color);
    border: none;
    background: none;
    padding: 0;
    transition: var(--moyo-transition-fast) color;
    cursor: pointer;
  }

  .input__clear:hover,
  .input__password-toggle:hover {
    color: var(--moyo-input-icon-color-hover);
  }

  .input__clear:focus,
  .input__password-toggle:focus {
    outline: none;
  }

  .input--empty .input__clear {
    visibility: hidden;
  }

  /* Don't show the browser's password toggle in Edge */
  ::-ms-reveal {
    display: none;
  }
`;

export {
  input_styles_default
};
